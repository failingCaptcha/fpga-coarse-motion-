`timescale 1ns / 1ps

module h_decim (
    input  logic               PCLK,
    input  logic               PRESETn,

    // Configuration
    input  logic [6:0][15:0]   h_filt_coeffs_i,
    input  logic [11:0]        h_size_i, 

    // Input Stream
    input  logic [7:0]         Y_in,
    input  logic [1:0]         valid_in,

    // Output Stream
    output logic [7:0]         Y_out,
    output logic [1:0]         valid_out
);

    logic [9:0] fifo_mem [0:15]; // {valid[1:0], Y[7:0]}
    logic [4:0] wr_ptr;
    logic [4:0] rd_ptr;
    
    wire fifo_full  = (wr_ptr == {~rd_ptr[4], rd_ptr[3:0]});
    wire fifo_empty = (wr_ptr == rd_ptr);
    wire pop        = !fifo_empty; //

    always_ff @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            wr_ptr <= 5'd0;
        end else if (valid_in != 2'b00 && !fifo_full) begin
            fifo_mem[wr_ptr[3:0]] <= {valid_in, Y_in};
            wr_ptr <= wr_ptr + 1'b1;
        end
    end

    // LUTRAM asynchronous read
    wire [9:0] fifo_rdata = fifo_mem[rd_ptr[3:0]];
    wire [7:0] Y_fifo     = fifo_rdata[7:0];
    wire [1:0] valid_fifo = fifo_rdata[9:8];

    always_ff @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            rd_ptr <= 5'd0;
        end else if (pop) begin
            rd_ptr <= rd_ptr + 1'b1;
        end
    end


    logic [7:0]  tap [0:6];
    logic [11:0] in_cnt;
    logic [1:0]  saved_start_valid;
    
    logic        trigger_pipe;
    logic [1:0]  pipe_valid_in;

    always_ff @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            for (int i = 0; i < 7; i++) tap[i] <= 8'd0;
            in_cnt            <= 12'd0;
            saved_start_valid <= 2'b00;
            trigger_pipe      <= 1'b0;
            pipe_valid_in     <= 2'b00;
        end else if (pop) begin
            if (valid_fifo[1] == 1'b1) begin 
                // valid == 2 or 3 (First pixel of the line)
                tap[0] <= Y_fifo;
                tap[1] <= Y_fifo;
                tap[2] <= Y_fifo;
                tap[3] <= Y_fifo;
                tap[4] <= 8'd0; 
                tap[5] <= 8'd0;
                tap[6] <= 8'd0;
                
                in_cnt            <= 12'd1;
                saved_start_valid <= valid_fifo; // Save the 2 or 3 flag
                trigger_pipe      <= 1'b0;
                pipe_valid_in     <= 2'b00;
            end else begin
                tap[0] <= tap[1];
                tap[1] <= tap[2];
                tap[2] <= tap[3];
                tap[3] <= tap[4];
                tap[4] <= tap[5];
                tap[5] <= tap[6];
                tap[6] <= Y_fifo;
                
                in_cnt <= in_cnt + 1'b1;

                // in_cnt == 3 becoming 4, 7 becoming 8, 11 becoming 12, etc.
                if (in_cnt[1:0] == 2'b11) begin
                    trigger_pipe  <= 1'b1;
                    pipe_valid_in <= (in_cnt == 12'd3) ? saved_start_valid : 2'b01; 
                end else begin
                    trigger_pipe  <= 1'b0;
                    pipe_valid_in <= 2'b00;
                end
            end
        end else begin
            trigger_pipe  <= 1'b0;
            pipe_valid_in <= 2'b00;
        end
    end


    
    logic signed [24:0] mult_res [0:6];
    logic [1:0]         pipe1_valid;

    always_ff @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            pipe1_valid <= 2'b00;
            for (int i = 0; i < 7; i++) mult_res[i] <= 25'd0;
        end else begin
            pipe1_valid <= trigger_pipe ? pipe_valid_in : 2'b00;
            if (trigger_pipe) begin
                for (int i = 0; i < 7; i++) begin
                    // Zero-extend unsigned 8-bit Y, then multiply by signed 16-bit coeff
                    mult_res[i] <= $signed({1'b0, tap[i]}) * $signed(h_filt_coeffs_i[i]);
                end
            end
        end
    end

    logic signed [27:0] add_res;
    logic [1:0]         pipe2_valid;

    always_ff @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            pipe2_valid <= 2'b00;
            add_res     <= 28'd0;
        end else begin
            pipe2_valid <= pipe1_valid;
            if (pipe1_valid != 2'b00) begin
                add_res <= mult_res[0] + mult_res[1] + mult_res[2] + mult_res[3] + 
                           mult_res[4] + mult_res[5] + mult_res[6];
            end
        end
    end

    always_ff @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            valid_out <= 2'b00;
            Y_out     <= 8'd0;
        end else begin
            valid_out <= pipe2_valid;
            if (pipe2_valid != 2'b00) begin
                automatic logic signed [27:0] rounded = add_res + 28'h00008000;
                automatic logic signed [27:0] shifted = rounded >>> 16;
                
                if (shifted < 0) begin
                    Y_out <= 8'd0;
                end else if (shifted > 255) begin
                    Y_out <= 8'd255;
                end else begin
                    Y_out <= shifted[7:0];
                end
            end
        end
    end

endmodule
